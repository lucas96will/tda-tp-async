    ...
    for largo_subarreglo in range(1, n + 1):
        for i in range(n - largo_subarreglo + 1):
            ...
            # Caso base: dos monedas
                ...
                sophia_izq_mateo_izq = (
                    coins[i] + opt[i + 2][j]
                    if coins[i + 1] >= coins[j] and i + 2 <= j
                    else 0
                )
                sophia_izq_mateo_der = (
                    coins[i] + opt[i + 1][j - 1]
                    if coins[i + 1] < coins[j] and i + 1 <= j - 1
                    else 0
                )
                ...
                sophia_der_mateo_izq = (
                    coins[j] + opt[i][j - 2]
                    if coins[i] < coins[j - 1] and i <= j - 2
                    else 0
                )
                sophia_der_mateo_der = (
                    coins[j] + opt[i + 1][j - 1]
                    if coins[i] >= coins[j - 1] and i + 1 <= j - 1
                    else 0
                )
...
def reconstruccion(coins, opt):
    ...
    while i <= j:
        ...
        sophia_der_mateo_izq = (
            coins[j] + opt[i][j - 2] if coins[i] < coins[j - 1] and i <= j - 2 else 0
        )
        sophia_der_mateo_der = (
            coins[j] + opt[i + 1][j - 1]
            if coins[i] >= coins[j - 1] and i + 1 <= j - 1
